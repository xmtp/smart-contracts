...
contract DeployGroupMessageBroadcaster is Script {
    ...
    function deployImplementation() public {
        require(Environment.GROUP_MESSAGE_BROADCASTER_IMPLEMENTATION != address(0), ImplementationNotSet());
        ...
        require(implementation_ == Environment.GROUP_MESSAGE_BROADCASTER_IMPLEMENTATION, UnexpectedImplementation());

        require(
            GroupMessageBroadcaster(implementation_).parameterRegistry() == Environment.PARAMETER_REGISTRY_PROXY,
    ...
    function deployProxy() public {
        require(Environment.GROUP_MESSAGE_BROADCASTER_PROXY != address(0), ProxyNotSet());
        require(Environment.FACTORY != address(0), FactoryNotSet());
        require(Environment.GROUP_MESSAGE_BROADCASTER_IMPLEMENTATION != address(0), ImplementationNotSet());
        ...
        (address proxy_, bytes memory constructorArguments_, ) = GroupMessageBroadcasterDeployer.deployProxy(
            Environment.FACTORY,
            Environment.GROUP_MESSAGE_BROADCASTER_IMPLEMENTATION,
            Environment.GROUP_MESSAGE_BROADCASTER_PROXY_SALT
        );

        require(proxy_ == Environment.GROUP_MESSAGE_BROADCASTER_PROXY, UnexpectedProxy());

        require(
            GroupMessageBroadcaster(proxy_).implementation() == Environment.GROUP_MESSAGE_BROADCASTER_IMPLEMENTATION,
            UnexpectedProxy()
        );
