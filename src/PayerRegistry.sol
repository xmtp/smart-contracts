...
contract PayerRegistry is IPayerRegistry, Initializable, UUPSUpgradeable, PausableUpgradeable {
    ...
    function finalizeWithdrawal(address recipient_) external whenNotPaused {
        ...
        require(
            uint32(block.timestamp) >= payer_.withdrawableTimestamp,
    ...
    function settleUsage(address[] calldata payers_, uint96[] calldata fees_) external onlySettler whenNotPaused {
        ...
        for (uint256 i; i < payers_.length; ++i) {
            address payer_ = payers_[i];
            uint96 fee_ = fees_[i];
        ...
        uint256 tokenBalance_ = IERC20Like(token).balanceOf(address(this));
        uint256 excess_ = tokenBalance_ > totalWithdrawable_ ? tokenBalance_ - totalWithdrawable_ : 0;
    ...
    function getActivePayers(
        ...
        for (uint256 i; i < count_; ++i) {
            activePayers_[i] = _activePayers().at(fromIndex_ + i);
        }
    }
    ...
    function getBalances(address[] calldata payers_) external view returns (int104[] memory balances_) {
        ...
        for (uint256 i; i < payers_.length; ++i) {
            balances_[i] = $.payers[payers_[i]].balance;
        }
    }
    ...
    function _increaseBalance(
        ...
        uint96 startingDebt_ = startingBalance_ < 0 ? uint96(uint104(-startingBalance_)) : 0;
        int104 endingBalance_ = ($.payers[payer_].balance = startingBalance_ + _toInt104(amount_));
        uint96 endingDebt_ = endingBalance_ < 0 ? uint96(uint104(-endingBalance_)) : 0;

        _activatePayerIfNecessary(payer_, endingBalance_, minActiveBalance_);

        return startingDebt_ - endingDebt_;
    }
    ...
    function _decreaseBalance(
        ...
        return endingDebt_ - startingDebt_;
    }
