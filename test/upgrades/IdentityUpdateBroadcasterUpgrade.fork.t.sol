...
import { IdentityUpdateBroadcasterUpgrader } from "../../script/upgrades/IdentityUpdateBroadcasterUpgrader.s.sol";
import { IMigratable } from "../../src/abstract/interfaces/IMigratable.sol";

contract IdentityUpdateBroadcasterUpgradeForkTest is Test {
    address constant admin = 0x560469CBb7D1E29c7d56EfE765B21FbBaC639dC7;

    Utils.DeploymentData internal deployment;
    IdentityUpdateBroadcasterUpgrader internal upgrader;

    function setUp() external {
        // Hardcoded environment and RPC - app-chain contracts live on L3 (xmtp_ropsten)
        string memory rpc = vm.rpcUrl("xmtp_ropsten");
        vm.createSelectFork(rpc);

        string memory environment = "testnet-dev";
        deployment = Utils.parseDeploymentData(string.concat("config/", environment, ".json"));

        upgrader = new IdentityUpdateBroadcasterUpgrader();
    }

    function test_identity_update_broadcaster() external {
        address factory = deployment.factory;
        address paramRegistry = deployment.parameterRegistryProxy;
        address proxy = deployment.identityUpdateBroadcasterProxy;

        // Get state before upgrade using script's getContractState
        IdentityUpdateBroadcasterUpgrader.ContractState memory stateBefore = upgrader.getContractState(proxy);
        ...
        if (computedImpl.code.length > 0) {
            ...
            (newImpl, ) = IdentityUpdateBroadcasterDeployer.deployImplementation(factory, paramRegistry);
        }
        ...
        vm.startPrank(admin);
        string memory key = IdentityUpdateBroadcaster(proxy).migratorParameterKey();
        ...
        // Execute migration
        IdentityUpdateBroadcaster(proxy).migrate();
        ...
        IdentityUpdateBroadcasterUpgrader.ContractState memory stateAfter = upgrader.getContractState(proxy);
