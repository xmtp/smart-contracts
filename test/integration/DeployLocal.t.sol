...
contract DeployLocalTests is Test {
    ...
    ISettlementChainParameterRegistry internal _parameterRegistry;
    MockUnderlyingFeeToken internal _underlyingFeeToken;
    ...
    function test_deployLocalProtocol() external {
        ...
        _parameterRegistry = _deploySettlementChainParameterRegistryProxy(parameterRegistryImplementation_, _admin);
        ...
        _underlyingFeeToken = _deployMockUnderlyingFeeTokenProxy(underlyingFeeTokenImplementation_);
        ...
        _feeToken = _deployFeeTokenProxy(feeTokenImplementation_);
        ...
        address rateRegistryImplementation_ = _deployRateRegistryImplementation(address(_parameterRegistry));

        _rateRegistry = _deployRateRegistryProxy(rateRegistryImplementation_);

        // Deploy the Node Registry.
        address nodeRegistryImplementation_ = _deployNodeRegistryImplementation(address(_parameterRegistry));
    ...
    function _setBroadcasterStartingParameters() internal {
        ...
        vm.prank(_admin);
        _parameterRegistry.set(keys_, values_);

        assertEq(_parameterRegistry.get(keys_[0]), values_[0]);
        assertEq(_parameterRegistry.get(keys_[1]), values_[1]);
        assertEq(_parameterRegistry.get(keys_[2]), values_[2]);
        assertEq(_parameterRegistry.get(keys_[3]), values_[3]);
    }

    function _assertBroadcasterStartingParameters() internal view {
        assertEq(
            uint256(_parameterRegistry.get(_GROUP_MESSAGE_BROADCASTER_MIN_PAYLOAD_SIZE_KEY)),
            _GROUP_MESSAGE_BROADCASTER_STARTING_MIN_PAYLOAD_SIZE
        );

        assertEq(
            uint256(_parameterRegistry.get(_GROUP_MESSAGE_BROADCASTER_MAX_PAYLOAD_SIZE_KEY)),
            _GROUP_MESSAGE_BROADCASTER_STARTING_MAX_PAYLOAD_SIZE
        );

        assertEq(
            uint256(_parameterRegistry.get(_IDENTITY_UPDATE_BROADCASTER_MIN_PAYLOAD_SIZE_KEY)),
            _IDENTITY_UPDATE_BROADCASTER_STARTING_MIN_PAYLOAD_SIZE
        );

        assertEq(
            uint256(_parameterRegistry.get(_IDENTITY_UPDATE_BROADCASTER_MAX_PAYLOAD_SIZE_KEY)),
